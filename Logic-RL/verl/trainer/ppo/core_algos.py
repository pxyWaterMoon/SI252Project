...
def compute_remax_outcome_advantage(token_level_rewards: torch.Tensor,
                                    greedy_token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   epsilon: float = 1e-6):
    """
    Compute advantage for GRPO, operating only on Outcome reward 
    ...
    non_zero_mask = (token_level_rewards != 0)
    scores = (token_level_rewards * non_zero_mask).sum(dim=-1)
    greedy_scores = (greedy_token_level_rewards * non_zero_mask).sum(dim=-1)
    ...
    with torch.no_grad():
        ...
        #         raise ValueError(f"no score in prompt index: {idx}")
        for i in range(bsz):
            scores[i] = (scores[i] - greedy_scores[i])
        scores = scores.unsqueeze(-1).tile([1, response_length]) * eos_mask
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                raise ValueError(f"no score in prompt index: {idx}")
        for i in range(bsz):
            k = len(id2score[index[i]])
            assert k > 1, f"index {index[i]} has only one score, which is not allowed in RLOO."
            scores[i] = scores[i] - (id2mean[index[i]] * k - scores[i]) / (k - 1 + epsilon) 
        scores = scores.unsqueeze(-1).tile([1, response_length]) * eos_mask
